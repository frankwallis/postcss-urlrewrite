...
var urlrewrite = function( config ) {
    ...
    var updateImport = function( atRule ) {
        ...
        var update = Promise.resolve(callback( helper.URI ));

        promises.push(update.then(function(){ atRule.params = helper.getModifiedRule() }));
    };
    ...
    var updateDecl = function( decl ) {
        ...
        var updates = helper.URIS.map(function(uri){ return Promise.resolve(callback(uri)) });

        promises.push(Promise.all(updates).then(function(){ decl.value = helper.getModifiedRule() }));
    };
